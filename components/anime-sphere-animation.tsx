...
const AnimeSphereAnimation = () => {
  ...
  const fitElementToParent = (el: HTMLElement, padding?: number) => {
    ...
    function resize() {
      ...
      anime.default.set(el, { scale: 1 }) // Changed from anime.set
      const pad = padding || 0
      ...
      timeout = setTimeout(() => anime.default.set(el, { scale: ratio }), 10) // Changed from anime.set
    }
  ...
  useEffect(() => {
    ...
    const breathAnimation = anime.default({
      // Changed from anime
      begin: () => {
    ...
    const introAnimation = anime.default // Changed from anime
      .timeline({
        autoplay: false,
      })
      .add(
        {
          ...
          strokeDashoffset: {
            value: [anime.default.setDashoffset, 0], // Changed from anime.setDashoffset
            duration: 3900,
            easing: "easeInOutCirc",
            delay: anime.default.stagger(190, { direction: "reverse" }), // Changed from anime.stagger
          },
          duration: 2000,
          delay: anime.default.stagger(60, { direction: "reverse" }), // Changed from anime.stagger
          easing: "linear",
        },
        0,
      )

    const shadowAnimation = anime.default({
      // Changed from anime
